...
async function main() {
  ...
  const mrBeatzHash = await bcrypt.hash('MrBeatz2024!', 10)
  const buyerHash = await bcrypt.hash('Buyer2024!', 10)
  ...
  // Mr. Beatz — the main producer account
  const mrBeatz = await prisma.user.create({
    data: {
      email: 'mrbeatz@wavr.com',
      password: mrBeatzHash,
      name: 'Mr. Beatz',
      role: 'producer',
      plan: 'producer_pro',
      verified: true,
      bio: 'Award-winning beat maker. Creating heat since day one. Every track hits different.',
    },
  })

  // Demo buyer / artist account
  const buyer = await prisma.user.create({
    data: {
      email: 'buyer@wavr.com',
      password: buyerHash,
      name: 'Demo Buyer',
      role: 'producer',
      plan: 'free',
      verified: true,
      bio: 'Artist looking for the perfect beat.',
    },
  })
  ...
  const beatData = [
    { title: 'Blue Flame', producerId: mrBeatz.id, genre: 'Trap', bpm: 142, key: 'B Min', mood: 'Dark', tags: JSON.stringify(['hard', 'melodic', 'cinematic']), priceBasic: 49.99, pricePremium: 149.99, priceExclusive: 499.99, plays: 38200, featured: true, status: 'active' },
    { title: 'Midnight Protocol', producerId: mrBeatz.id, genre: 'Hip-Hop', bpm: 94, key: 'G Min', mood: 'Motivated', tags: JSON.stringify(['boom bap', 'hard', 'classic']), priceBasic: 39.99, pricePremium: 129.99, priceExclusive: 399.99, plays: 21500, featured: true, status: 'active' },
    { title: 'Electric Storm', producerId: mrBeatz.id, genre: 'Drill', bpm: 145, key: 'D# Min', mood: 'Aggressive', tags: JSON.stringify(['drill', 'dark', 'energy']), priceBasic: 59.99, pricePremium: 179.99, priceExclusive: 599.99, plays: 44800, featured: true, status: 'active' },
    { title: 'Neon Frequency', producerId: mrBeatz.id, genre: 'R&B', bpm: 86, key: 'A Maj', mood: 'Chill', tags: JSON.stringify(['smooth', 'vibes', 'melodic']), priceBasic: 34.99, pricePremium: 109.99, priceExclusive: 349.99, plays: 17300, featured: false, status: 'active' },
  ...
  console.log(`\nSeed complete! Created: 1 admin, 5 producers, 1 buyer, ${beats.length} beats, ${merchProducts.length} merch`)
  console.log('\n=== LOGIN CREDENTIALS ===')
  console.log('  [ADMIN]    admin@wavr.com        / admin123')
  console.log('  [PRODUCER] mrbeatz@wavr.com      / MrBeatz2024!')
  console.log('  [PRODUCER] producer@wavr.com     / password123')
  console.log('  [PRODUCER] wavegod@wavr.com      / password123')
  console.log('  [PRODUCER] afroking@wavr.com     / password123')
  console.log('  [BUYER]    buyer@wavr.com        / Buyer2024!')
  console.log('=========================\n')
