...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const genres = form.get("genres") as string | null;
      if (genres) data.genres = genres;
      const moods = form.get("moods") as string | null;
      if (moods) data.moods = moods;
    } else {
      ...
      if (body.socialLinks !== undefined) { try { JSON.parse(body.socialLinks); data.socialLinks = body.socialLinks; } catch {} }
      if (body.genres !== undefined) data.genres = body.genres;
      if (body.moods !== undefined) data.moods = body.moods;
    }
    ...
    return NextResponse.json({
      ...
      printfulStoreId: updated.printfulStoreId, tagAudio: updated.tagAudio,
    });
  } catch (err) {
