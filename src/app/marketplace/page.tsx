        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative max-w-xl">
            ...
            <input
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-xl pl-10 pr-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm"
            />
          </div>
        </div>
      </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin">
              {featuredBeats.map((beat) => (
                <div
                  key={beat.id}
                  className="shrink-0 w-64 bg-gray-800 border border-gray-700 rounded-xl p-4 hover:border-blue-500/50 cursor-pointer transition-all duration-200 group"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-blue-500 text-xs font-bold">from ${beat.priceBasic}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          `}>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-6">
              ...
              <div>
                <h3 className="text-white font-semibold text-sm mb-3">
                  Max Basic Price:{" "}
                  <span className="text-blue-500 font-bold">${priceMax}</span>
                </h3>
                  <input
                    type="range"
                    min={10}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="bg-gray-800 border border-gray-700 text-gray-300 text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-blue-500"
                >
                ...
                <div className="flex bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
                  <button
                    ...
                    title="Grid view"
                    className={`px-3 py-2 transition-colors ${view === "grid" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-300"}`}
                  >
                  ...
                  <button
                    ...
                    title="List view"
                    className={`px-3 py-2 transition-colors ${view === "list" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-300"}`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {loading ? (
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[2fr_1fr_1fr_auto] gap-4 px-4 py-2.5 border-b border-gray-800 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                  <span>Beat</span>
                ...
                {filteredBeats.map((beat, idx) => {
                  ...
                  return (
                    <div
                      key={beat.id}
                      className={`grid grid-cols-[2fr_1fr_1fr_auto] gap-4 px-4 py-3 items-center hover:bg-gray-800/50 transition-colors ${idx < filteredBeats.length - 1 ? "border-b border-gray-800/60" : ""}`}
                    >
                      {/* Artwork + title */}
                      <div className="flex items-center gap-3 min-w-0">
                        <button
                          onClick={() => setCurrentBeat(beat)}
                          className={`w-10 h-10 rounded-lg shrink-0 relative overflow-hidden ${beat.artwork ? "" : "bg-gradient-to-br from-blue-600 to-blue-900"} flex items-center justify-center`}
                        >
                          {beat.artwork && (
                            <img src={beat.artwork} alt="" className="absolute inset-0 w-full h-full object-cover" />
                          )}
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-gray-500 text-xs truncate">{beat.producer}</p>
                        </div>
                        {beat.featured && (
                          <span className="hidden md:inline-flex shrink-0 text-[10px] px-2 py-0.5 rounded-full bg-amber-900/30 text-amber-400 border border-amber-700/30 font-semibold">Featured</span>
                        )}
                      </div>

                      {/* Info */}
                      <div className="hidden sm:block">
                        <p className="text-gray-400 text-xs">{beat.genre}</p>
                        <p className="text-gray-600 text-xs">{beat.bpm} BPM · {beat.key}</p>
                      </div>

                      {/* Price */}
                      <p className="text-blue-400 text-sm font-bold text-right">from ${beat.priceBasic}</p>
