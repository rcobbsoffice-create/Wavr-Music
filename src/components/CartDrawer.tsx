...
export default function CartDrawer() {
  ...
  return (
    <>
      {/* Cart button — sits above the SupportAI chat widget */}
      <button
        onClick={() => setOpen(true)}
        className="fixed bottom-24 right-6 z-40 bg-blue-700 hover:bg-blue-600 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg shadow-blue-900/20 transition-colors"
            ...
            items.map((item) => (
              <div key={`${item.productId}-${item.size}`} className="flex gap-3 bg-gray-800 rounded-xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-blue-400 text-sm font-semibold mt-1">${item.price.toFixed(2)}</p>
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button
                      ...
                      className="ml-auto text-gray-500 hover:text-blue-400 text-xs"
                    >Remove</button>
                  </div>
                </div>
              </div>
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-gray-800 space-y-3">
            ...
            <button
              ...
              disabled={checking}
              className="w-full bg-gradient-to-r from-blue-600 to-blue-400 hover:from-blue-600 hover:to-blue-400 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all"
            >
