    ...
    <header className="bg-gray-950/50 backdrop-blur-md border-b border-gray-800 h-16 flex items-center px-4 sm:px-6 lg:px-8 sticky top-0 z-30">
          ...
          <div className="hidden md:flex flex-1 max-w-md ml-4">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="w-full bg-gray-900 border border-gray-800 rounded-xl pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-500 transition-all"
              />
            </div>
          </div>
        )}
      ...
      <div className="flex items-center gap-3 ml-4">
        ...
        <div className={`w-8 h-8 rounded-full border border-gray-800 flex items-center justify-center text-xs font-bold text-white bg-gradient-to-br ${
          user?.role === 'admin' ? 'from-blue-600 to-orange-600' : 'from-blue-600 to-blue-400'
        }`}>
          {user?.name?.charAt(0)}
        </div>
      </div>
