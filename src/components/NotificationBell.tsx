  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 bg-blue-700 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-8 w-80 bg-gray-900 border border-gray-800 rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
            ...
            {unread > 0 && (
              <button onClick={markAllRead} className="text-blue-400 hover:text-blue-400 text-xs">
                Mark all read
              </button>
            )}
          </div>
            ...
            {loading ? (
              ...
              notifications.map((n) => (
                <div
                  ...
                  className={`flex gap-3 px-4 py-3 border-b border-gray-800/50 last:border-0 cursor-pointer hover:bg-gray-800/50 transition-colors ${!n.read ? "bg-blue-900/30" : ""}`}
                >
                  ...
                  {!n.read && <div className="w-2 h-2 rounded-full bg-blue-700 shrink-0 mt-1.5" />}
                </div>
              ))
            )}
          </div>
