...
const STEM_LABELS: Record<string, { label: string; color: string; emoji: string }> = {
  drums:  { label: "Drums",  color: "bg-blue-600",    emoji: "🥁" },
  bass:   { label: "Bass",   color: "bg-blue-600",   emoji: "🎸" },
  melody: { label: "Melody", color: "bg-blue-700", emoji: "🎹" },
        ...
        <div className="flex items-center gap-2">
          {readyStems.some((s) => s.filePath) && (
            <button
              ...
              className="text-xs bg-blue-700 hover:bg-blue-700 text-white px-3 py-1 rounded-lg flex items-center gap-1.5"
            >
              {playing ? (
                <><span>⏸</span> Pause</>
              ) : (
          ...
          <div className="flex flex-wrap gap-2">
            {readyStems.filter((s) => s.filePath).map((s) => (
              <a
                ...
                download
                className="text-xs text-blue-400 hover:text-blue-400 bg-blue-900/30 border border-blue-700/30 px-3 py-1 rounded-lg"
              >
